...
const bank = [
  mem({ id: 'promise_lamp', type: 'promise', summary: 'Promised to return the lamp to Pell.', triggerTerms: ['lamp', 'pell'] }),
  mem({ id: 'promise_old', type: 'promise', summary: 'Kept promise.', triggerTerms: ['bridge'], status: 'retired' }),
  mem({ id: 'obj_lamp', type: 'object', summary: 'A brass lamp.', triggerTerms: ['lamp', 'old mill'] }),
  mem({ id: 'obj_key', type: 'object', summary: 'A tiny key.', triggerTerms: ['key'], sourceIds: ['place_mill'] }),
  mem({ id: 'obj_secret', type: 'object', summary: 'Hidden.', triggerTerms: ['old mill'], privacy: 'private' }),
  mem({ id: 'place_mill', type: 'place', summary: 'The old mill.', triggerTerms: ['old mill'] }),
  mem({ id: 'char_pell', type: 'character', summary: 'Pell.', triggerTerms: ['pell'] })
];
...
describe('N06 promise/object/place continuity', () => {
  ...
  it('resolving a promise closes it but keeps the history card', () => {
    ...
    expect(continuityDigest([resolved]).openPromises).toEqual([]);
